DROP PACKAGE DSS.PKG_SERIAL_MASTER3;

CREATE OR REPLACE PACKAGE DSS.PKG_SERIAL_MASTER3
AS
   /*
   ...
   */
   ...
   p_LAST_RUN_DATE   DATE DEFAULT SYSDATE -2; --- (365 * 3);
   ...
   IS
      SELECT SN.SERIALKEY,
       ...
       WHERE     1 = 1
             ...
             and              SN.PRODUCT_CODE is not null  
...
END PKG_SERIAL_MASTER3;
/


DROP PACKAGE BODY DSS.PKG_SERIAL_MASTER3;

CREATE OR REPLACE PACKAGE BODY DSS.PKG_SERIAL_MASTER3
AS
            ...
            FORALL RW IN 1 .. TEMP_RW.COUNT
               MERGE INTO DSS_SERIALS_MASTER TRG
                USING (SELECT TEMP_RW (RW).SERIALKEY AS SERIALKEY,
                         ...
                         FROM DUAL where TEMP_RW (RW).PRODUCT_CODE is not null ) MTCH
                   ON (    MTCH.SERIALKEY = TRG.SERIALKEY
...
GRANT EXECUTE, DEBUG ON DSS.PKG_SERIAL_MASTER3 TO XXREPDSS;
