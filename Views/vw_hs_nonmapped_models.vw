DROP VIEW DSS.VW_HS_NONMAPPED_MODELS;

CREATE OR REPLACE FORCE VIEW DSS.VW_HS_NONMAPPED_MODELS
(
    OPERATINGUNIT
   ,ORGANIZATIONID
   ,ORGANIZATION_NAME
...
AS
    ...
    WHERE 1 = 1
          ...
          AND S.ORGANIZATION_ID != 2338
          AND O.OPERATING_UNIT NOT IN ('2258')   -- added by vijay 25-mar-2020
          AND m.CREATE_DATE >= SYSDATE - 750     -- added by vijay 25-mar-2020
          --          AND p.CATEGORY_CODE NOT IN ('02', '06')
          --          AND p.CATEGORY_CODE != '02'
          --          AND p.CATEGORY_CODE != '06'
          AND p.CATEGORY_CODE = '01'
          AND M.VMODEL_SEQ IS NULL
          AND P.MODEL_CODE != 'P2BEATHS'
          AND P.MODEL_CODE != 'P2HLTDEV'
          AND P.MODEL_CODE != 'CAMCOM6'
          AND TRUNC(s.CREATE_DATE) >= TRUNC(SYSDATE - 800)
    ORDER BY 1, 2, 3;
